...
public class ReclamationService implements IReclamationService {
    ...
    public List<ReclamationResponseDTO> getByEntreprise(Long entrepriseId) {
        List<Reclamation> reclamations = reclamationRepo.findByAuthor_Entreprise_Id(entrepriseId);
    ...
    public List<ReclamationResponseDTO> getByStatusAndEntreprise(ReclamationStatus status, Long entrepriseId) {
        return reclamationRepo.findByStatusAndAuthor_Entreprise_Id(status, entrepriseId)
                .stream()
                .map(this::mapToDTO)
                .collect(Collectors.toList());
    }
    ...
    public long countReclamationsByEntreprise(Long entrepriseId) {
        return reclamationRepo.countByAuthor_Entreprise_Id(entrepriseId);
    }

    public long countResolvedReclamationsByEntreprise(Long entrepriseId) {
        return reclamationRepo.countByStatusAndAuthor_Entreprise_Id(ReclamationStatus.RESOLVED, entrepriseId);
    }

    public Long countInProgressReclamationsByEntreprise(Long entrepriseId) {
        return reclamationRepo.countByStatusAndAuthor_Entreprise_Id(ReclamationStatus.IN_PROGRESS, entrepriseId);
    }
